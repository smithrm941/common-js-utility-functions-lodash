const ceil = (number, precision) => {
    ...
    let afterDecimal = numberString.substring(
        decimalIndex,
        numberString.length
    );
    ...
    if (precision === undefined || precision === 0) {
        for (let i = 0; i < afterDecimal.length; i++) {
            if (numberString[i] > 0) {
                let roundUp = parseInt(beforeDecimal) + 1;
                return roundUp;
            }
        }
    }
    if (precision > 0) {
        ...
        for (let j = 1; j < afterDecimalArray.length; j++) {
            if (j === precision) {
                afterDecimalArray.splice(
                    j,
                    1,
                    parseInt(afterDecimalArray[j]) + 1
                );
        ...
        let roundedFloat = newNumberString.substring(
            0,
